...
class Communicator:
    ...
    def __init__(self, config, sensors):
        ...
        mqtt_keepalive = (
            int(self.conf["mqtt_keepalive"]) if "mqtt_keepalive" in self.conf else 60
        )
        ...
        logging.debug(
            "Connecting to host %s, port %s, keepalive %s",
            self.conf["mqtt_host"],
            mqtt_port,
            mqtt_keepalive,
        )
    ...
    def _on_disconnect(self, _mqtt_client, _userdata, _flags, reason_code, _properties):
        ...
        if reason_code == 0:
            ...
            logging.warning(
                "Unexpectedly disconnected from MQTT broker: %s", reason_code
            )
    ...
    def _on_mqtt_message(self, _mqtt_client, _userdata, msg):
        ...
        if not found_topic:
            logging.warning(
                "Unexpected or erroneous MQTT message: %s: %s", msg.topic, msg.payload
            )

    def _on_mqtt_publish(
        self, _mqtt_client, _userdata, _mid, _reason_codes, _properties
    ):
                ...
                if prop == "send":
                    ...
                    try:
                        ...
                        try:
                            ...
                            logging.warning(
                                "Cannot parse int nor float value for %s: %s",
                                msg.topic,
                                msg.payload,
                            )
        ...
        for cur_sensor in self.sensors:
            if cur_sensor["name"] + "/" in mqtt_topic:
                ...
                if prop == "req":
                    ...
                    logging.debug(
                        "%s: %s=%s", cur_sensor["name"], prop, mqtt_json_payload
                    )
                    if "data" not in cur_sensor:
                        cur_sensor["data"] = {}
    ...
    def _send_message(self, sensor, clear):
        ...
        destination = [(sensor["address"] >> i * 8) & 0xFF for i in reversed(range(4))]
        ...
        if command_shortcut:
            ...
            if (
                command_shortcut not in sensor["data"]
                or sensor["data"][command_shortcut] is None
            ):
                logging.warning(
    ...
    def _read_packet(self, packet):
        ...
        for cur_sensor in self.sensors:
            ...
            if enocean.utils.combine_hex(packet.sender) == cur_sensor["address"]:
                ...
                if not packet.learn or str(cur_sensor.get("log_learn")) in (
                    "True",
                    "true",
                    "1",
                ):
    ...
    def _reply_packet(self, in_packet, sensor):
        ...
        self._send_packet(
            sensor, destination, None, True, in_packet.data if in_packet.learn else None
        )
    ...
    def _send_packet(
        ...
        if not is_learn:
            ...
            if "default_data" in sensor:
                packet.data[1:5] = [
                    (sensor["default_data"] >> i * 8) & 0xFF for i in reversed(range(4))
                ]
    ...
    def _process_radio_packet(self, packet):
        ...
        for cur_sensor in self.sensors:
            if (
                "address" in cur_sensor
                and enocean.utils.combine_hex(packet.sender) == cur_sensor["address"]
            ):
        ...
        if not found_sensor:
            logging.info(
                "unknown sensor: %s", enocean.utils.to_hex_string(packet.sender)
            )
            return
